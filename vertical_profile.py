...
def plot_scale_height_rms(timesteps, r, z, m, sz, disk_mask, stride, num_bins, beta, run): 
    ...
    fname = f'/scale-height-rms/scale_height_rms_1e6_{run}_{beta}_25.pdf'
    ...
    plt.grid()
    plt.legend(loc='upper right', fontsize='small')
    fname = f'/ar-rms/ar_rms_1e6_{run}_{beta}_25.pdf'
    plt.savefig(plots + fname)
    plt.show()
...
def plot_scale_height_density(timesteps, r, z, rho, disk_mask, stride, num_r_bins, num_z_bins, beta, run):
    ...
    plt.grid()
    plt.legend()
    fname_scale = f'/scale-height/scale_heights_rho_1e6_{run}_{beta}_25.pdf'
    ...
    plt.grid()
    plt.legend()
    fname_aspect = f'/ar-rho/aspect_ratios_rho_1e6_{run}_{beta}_25.pdf'
...
def plot_aspect_ratio(timesteps, c_s, r, m_s, disk_mask, stride, num_bins, beta, run): 
    ...
    for t in timesteps: 
        ...
        plt.grid()
        plt.legend()
        fname = f'/ar-cs/aspect_ratios_cs_1e6_{run}_{beta}_25.pdf'
        plt.savefig(plots + fname)  
        plt.show()
