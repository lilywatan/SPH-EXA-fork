#!/usr/bin/env bash
#SBATCH --job-name=disk_subgrid_beta_planet_star0         # Job name    (default: sbatch)
#SBATCH --output=disk_subgrid_beta_planet_star0.out        # Output file (default: slurm-%j.out)
#SBATCH --error=disk_subgrid_beta_planet_star0.err         # Error file  (default: slurm-%j.err)
#SBATCH --cpus-per-task=32       # Number of CPUs per task
#SBATCH --ntasks=1                # Number of tasks
#SBATCH --ntasks-per-node=1      # Number of tasks per node§
#SBATCH --mem-per-cpu=1G          # Memory per CPU
#SBATCH --time=00:80:00           # Wall clock time limit
...
srun $EXEC_PATH --init '/home/lwatan/data/SPH-EXA-fork/subgrid_init_planet_star0.hdf5' --prop std-subgrid -s 1e6 -w 1000 -f m,c,x,y,z,rho,p,vx,vy,vz,h -o '/home/lwatan/scratch/run_subgrid_beta_planet_star0.hdf5'
