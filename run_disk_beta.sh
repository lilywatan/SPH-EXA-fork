#!/usr/bin/env bash
#SBATCH --job-name=disk_mom_50_beta         # Job name    (default: sbatch)
#SBATCH --output=disk_mom_50_beta.out        # Output file (default: slurm-%j.out)
#SBATCH --error=disk_mom_50_beta_%j.err         # Error file  (default: slurm-%j.err)
#SBATCH --cpus-per-task=32        # Number of CPUs per task
#SBATCH --ntasks=1                # Number of tasks
#SBATCH --ntasks-per-node=1      # Number of tasks per node§
#SBATCH --mem-per-cpu=1G          # Memory per CPU
#SBATCH --time=50:00:00           # Wall clock time limit
...
srun build-release/main/src/sphexa/sphexa --init '/home/lwatan/data/disk5_beta.hdf5' --prop std-angmom -s 50000 -w 10 -f m,c,x,y,z,rho,p,vx,vy,vz,h -o '/home/lwatan/data/SPH-EXA-fork/output/runs/run_disk_mom_50_beta.hdf5'
