#!/usr/bin/env bash
#SBATCH --job-name=disk_mom_1e6_1         # Job name    (default: sbatch)
#SBATCH --output=disk_mom_1e6_1.out        # Output file (default: slurm-%j.out)
#SBATCH --error=disk_mom_1e6_1.err         # Error file  (default: slurm-%j.err)
...
echo "26-05-2025: Running SPH-EXA with 1e6 timesteps, momentum criterion, beta = 2pi, h = 1.2, star_inner = 1.0, 2.0 angmom threshold"
srun build-mom-double/main/src/sphexa/sphexa --init '/home/lwatan/data/disk5_beta.hdf5' --prop std-angmom -s 8000 -w 10 -f c,m,x,y,z,rho,vx,vy,vz,h,u,temp,alpha,du_m1,x_m1,y_m1,z_m1 -o '/home/lwatan/scratch/run_disk_mom_double_2.hdf5'