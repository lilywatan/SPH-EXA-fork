#!/usr/bin/env bash
#SBATCH --job-name=disk_radial_100_beta       # Job name    (default: sbatch)
#SBATCH --output=disk_radial_100_beta_%j.out        # Output file (default: slurm-%j.out)
#SBATCH --error=disk_radial_100_beta_%j.err         # Error file  (default: slurm-%j.out)
#SBATCH --cpus-per-task=32         # Number of CPUs per task
#SBATCH --ntasks=1                # Number of tasks
#SBATCH --ntasks-per-node=1      # Number of tasks per node§
#SBATCH --mem-per-cpu=1G          # Memory per CPU
#SBATCH --time=30:00:00           # Wall clock time limit
...
srun build-release/main/src/sphexa/sphexa --init '/home/lwatan/data/disk5_beta.hdf5' --prop std-planet -s 100000 -w 10 -f m,c,x,y,z,rho,p,vx,vy,vz,h -o '/home/lwatan/scratch/run_disk_radial_100_beta.hdf5'
