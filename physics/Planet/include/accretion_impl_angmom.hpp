...
void computeAccretionConditionImplAngMom(size_t first, size_t last, Dataset& d, StarData& star)
{
    ...
    const double frac = 2.0; // fraction of escape angular momentum to consider
...
#pragma omp parallel for reduction(+ : accr_mass) reduction(+ : accr_mom[ : 3]) reduction(+ : n_accreted)              \
    ...
    for (size_t i = first; i < last; i++)
    {
        ...
        // combined accretion criterion: angular momentum and distance
        //if (J2 < J_esc2 && dist2 < star_size2) { remove_and_sum(i, accr_mass, accr_mom, n_accreted); }

        // only angular momentum criterion: 
